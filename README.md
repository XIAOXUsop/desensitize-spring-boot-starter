...
密钥由应用在仓库外管理，不能与令牌 HMAC 密钥共用。`rotateKey(newKey)` 会在文件锁下重加密；多进程部署时先协调所有实例的密钥切换，旧密钥实例会解密失败。密文损坏或密钥错误会使保险库拒绝启动或操作，不会猜测原文。本文件实现适合单机参考；跨主机共享与备份恢复需由业务侧接数据库/KMS 实现。生产仍需：
...
114 项测试覆盖：8 种脱敏类型的算法与边界（含 `keepFirst` / `keepLast` 越界参数的钳制）、

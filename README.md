...
> 那条在 0.6.3 里没有回退。该版本发版前实测：`mvn -o -B test` → 一百一十项，0 失败。
...
`TokenVault` 默认给的是内存实现（进程重启即丢）。需要跨重启恢复时，可显式构造 `EncryptedFileTokenVault`；它在文件锁下读写 AES-GCM 密文，写入采用原子替换，`forget(scope)` 返回后该作用域不能再还原。

```java
byte[] keyBytes = Base64.getDecoder().decode(System.getenv("VAULT_AES_KEY_B64"));
SecretKey vaultKey = new SecretKeySpec(keyBytes, "AES");
ScopedTokenVault scopedVault = new EncryptedFileTokenVault(
        Path.of("/var/lib/my-app/token-vault.bin"), vaultKey, Duration.ofHours(2));
```

密钥由应用在仓库外管理，不能与令牌 HMAC 密钥共用。`rotateKey(newKey)` 会在文件锁下重加密；多进程部署时先协调所有实例的密钥切换，旧密钥实例会解密失败。密文损坏或密钥错误会使保险库拒绝启动或操作，不会猜测原文。文件上限为 64 MiB，超限写入会拒绝并保留原文件。本文件实现适合单机参考；跨主机共享与备份恢复需由业务侧接数据库/KMS 实现。生产接入还需核对：

- 由业务侧安全保存 AES 密钥，并与令牌 HMAC 密钥**分开管理**
- 把 `RestoreAudit` 回调接入持久审计日志，记录操作者与时间；回调本身不含原文
- 把会话/租户/客户映射到 `VaultScope`，按业务保留期调用 `forget(scope)`
- 为跨主机共享、备份恢复与多实例密钥轮换另接数据库/KMS；本文件实现不提供这些能力
...
> 内存实现终究靠进程重启兜底，TTL 只是让"忘了清"有个上限。需要跨重启恢复时显式注入加密文件实现或业务自己的持久化实现。
...
115 项测试覆盖：8 种脱敏类型的算法与边界（含 `keepFirst` / `keepLast` 越界参数的钳制）、
...
审计回调不含原文）；**加密持久保险库**（重启恢复、并发实例、作用域撤销、过期、碰撞、密钥轮换与损坏密文拒绝还原）；**模型调用装饰器**（多轮令牌一致、未知令牌不猜测、异常信息消毒、
...
- [x] 可选的单机加密文件保险库（`EncryptedFileTokenVault`，含过期、作用域撤销与密钥轮换）
- [ ] 跨主机共享、备份恢复与 KMS 托管的保险库实现
